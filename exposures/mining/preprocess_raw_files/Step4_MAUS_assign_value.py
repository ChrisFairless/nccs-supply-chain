...
def get_mining_exp(countries=None,
    ...
    # data = pd.read_hdf(r"C:\github\nccs-correntics\mining\intermediate_data_MAUS\global_miningarea_v2_5arcminute_converted_ISO3_improved.h5") #low res
    data = pd.read_hdf(r"C:\github\nccs-correntics\mining\intermediate_data_MAUS\global_miningarea_v2_30arcsecond_converted_ISO3_improved.h5")
    cnt_dfs = []
...
def get_ROW_factor_GDP(mriot_year, IO_countries, countries):
    ...
    gdp_worldbank = pd.read_csv(r'C:\github\nccs-correntics\mining\preprocess_raw_files\GDP_Worldbank_modified_without_regions.csv')
...
def get_ROW_factor_mineral_rent_GDP(mriot_year, IO_countries, countries):
    ...
    #load the GDP of counries
    gdp_worldbank = pd.read_csv(r'C:\github\nccs-correntics\mining\preprocess_raw_files\GDP_Worldbank_modified_without_regions.csv')
    mineral_rent = pd.read_csv(r'C:\github\nccs-correntics\mining\preprocess_raw_files\WorldBank_mineral_rents_modified_without_regions.csv')

    # Select only the specified year column and filter rows based on the 'Country Code',
    # select only the countries with are not within the IO table
    ROW_gdp_worldbank = gdp_worldbank[['Country Code', str(mriot_year)]][~gdp_worldbank['Country Code'].isin(IO_countries)]
    ROW_mineral_rent_worldbank = mineral_rent[['Country Code', str(mriot_year)]][~mineral_rent['Country Code'].isin(IO_countries)]

    # select only the countries which are in the countries list (and not in the IO from before)
    filtered_gdp_worldbank = ROW_gdp_worldbank[ROW_gdp_worldbank['Country Code'].isin(countries)]
    filtered_mineral_rent_worldbank = ROW_mineral_rent_worldbank[ROW_mineral_rent_worldbank['Country Code'].isin(countries)]
    ...
    # Create a new column with normalized GDP values
    mineral_production_worldbank['Normalized_production'] = mineral_production_worldbank[f'{str(mriot_year)}_mineral_production'] / ROW_total_mineral_prodoction
...
df_shape.to_file(r"C:\github\nccs-correntics\mining\intermediate_data_MAUS\global_miningarea_v2_30arcsecond_converted_ISO3_improved_values_MP_scaled.shp", driver="ESRI Shapefile")

#Save final file to a climada available format h5
df = exp.gdf.drop(columns=["geometry", "area", "normalized_area"])
df.to_hdf(r"C:\github\nccs-correntics\mining\intermediate_data_MAUS\global_miningarea_v2_30arcsecond_converted_ISO3_improved_values_MP_scaled.h5", key="data", mode="w") #hih res
...
plt.tight_layout()
plt.savefig(
    r'C:\github\nccs-correntics\mining\intermediate_data_MAUS\global_miningarea_v2_30arcsecond_converted_ISO3_improved_values_MP_scaled.png',
    bbox_inches='tight')
plt.show()
