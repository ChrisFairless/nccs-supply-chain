...
for io_approach in io_approaches:
    ...
    data_files = glob.glob(f"{get_direct_output_dir(RUN_TITLE)}/direct_impacts_*{io_approach}*.csv")
    dfs = []
    for filename in data_files:
        df = pd.read_csv(filename)
        df['country_of_impact_iso_a3'] = filename.split("_")[-1].split(".")[0]
    ...
    # Write to Excel with multiple sheets
    output_path = f"{get_direct_output_dir(RUN_TITLE)}/{RUN_TITLE}_direct_impacts_complete_{io_approach}.xlsx"
    with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
        # Write the main data to the first sheet
